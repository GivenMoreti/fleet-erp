...
-- Trigger to ensure at least one of user_certificate_id or pre_start_check_id is not null
DELIMITER //
CREATE TRIGGER compliance_check_before_insert BEFORE INSERT ON compliance
FOR EACH ROW
BEGIN
    IF NEW.user_certificate_id IS NULL AND NEW.pre_start_check_id IS NULL THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Either user_certificate_id or pre_start_check_id must be set';
    END IF;
END//

CREATE TRIGGER compliance_check_before_update BEFORE UPDATE ON compliance
FOR EACH ROW
BEGIN
    IF NEW.user_certificate_id IS NULL AND NEW.pre_start_check_id IS NULL THEN
        SIGNAL SQLSTATE '45000'
